...
def split_str_with_dot_and_semicolon(word_meaning_str:str) -> list:
    ...
    return split_list
def translate_guideword_to_guideword_cn():
    #将guideword翻译成中文guideword_cn，将guideword_cn写入Excel
    df_word_level = pd.read_excel('apple_european_standard_level.xlsx')
    ...
    print("****************************************************开始翻译*********************************************")
    for word_index in trange(len(guide_word_list)):
        # print(word_nan)
        #如果当前单词是nan
        if guide_word_list[word_index]=='nan':
            trans_guide_word_list.append("")
        else:
            trans_guide_word_list.append(Obj.tran(guide_word_list[word_index]))
            # chinese_text=translate_text(guide_word_list[word_index].lower(), 'zh-cn')
            # trans_guide_word_list.append(chinese_text)
    print("****************************************************trans_guide_word_lis*************************************")
    print(trans_guide_word_list)

    #将trans_guide_word_list写入Excel
    # 将列表转换为Pandas的数据框
    print("****************************************************将列表转换为Pandas的数据框***********************************")
    df_word_level ['guideword_cn']=trans_guide_word_list

    # 将数据框写入Excel文件
    print("****************************************************将数据框写入Excel文件***********************************")
    df_word_level.to_excel('apple_european_standard_level.xlsx', index=False)
    print("----------------------------------翻译OK!!!-----------------------------------------------------------------")

def generate_chinese_meaning():
    '''
...
if __name__ == '__main__':
    ...
    translate_guideword_to_guideword_cn()
